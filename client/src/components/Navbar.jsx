    ...
    return (
        <nav className="bg-white shadow-md p-4 mb-4 relative z-50">
            <div className="container mx-auto flex justify-between items-center">
                ...
                {/* LOGO - Adding 'notranslate' protects it from translation engines */}
                ...
                <div className="hidden md:flex items-center gap-6 font-bold text-sm uppercase tracking-wide">
                    <Link to={menuLink} className="hover:text-primary">{t('menu') || 'Menu'}</Link>
                    ...
                    {user ? (
                        <>
                            ...
                            {['admin', 'staff'].includes(user.role) && (
                                <>
                                    <Link to="/waiter" className="text-blue-600 hover:opacity-80">My Station</Link>
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden absolute top-full left-0 right-0 bg-white border-t shadow-lg flex flex-col p-4 gap-4 font-bold text-gray-700 uppercase z-50">
                    ...
                    <Link to={menuLink} onClick={() => setIsMobileMenuOpen(false)}>Menu</Link>
                    
                    {user ? (
                        <>
                            {['admin', 'kitchen'].includes(user.role) && (
                                <>
                                    <Link to="/kitchen" onClick={() => setIsMobileMenuOpen(false)}>Kitchen</Link>
                                    <Link to="/manage-menu" onClick={() => setIsMobileMenuOpen(false)}>Menu Mgr</Link>
                                </>
                            )}
                            
                            {['admin', 'staff'].includes(user.role) && (
                                <>
                                    <Link to="/waiter" className="text-blue-600" onClick={() => setIsMobileMenuOpen(false)}>My Station</Link>
                                    <Link to="/manage-tables" className="text-blue-600" onClick={() => setIsMobileMenuOpen(false)}>Tables</Link>
                                </>
                            )}
                            
                            {user.role === 'admin' && (
                                <Link to="/staff" className="text-secondary" onClick={() => setIsMobileMenuOpen(false)}>Staff</Link>
                            )}

                            <div className="border-t pt-4 mt-2 flex flex-col gap-2">
                                <span className="text-xs text-gray-400 normal-case">{user.email}</span>
                                <button onClick={() => { logout(); setIsMobileMenuOpen(false); }} className="text-left text-red-500">Logout</button>
                            </div>
                        </>
                    ) : (
                        <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="text-primary">Login</Link>
                    )}
                </div>
            )}
        </nav>
